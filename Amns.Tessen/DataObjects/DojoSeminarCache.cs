...
namespace Amns.Tessen
{
	...
	/// </summary>
	/// </summary>
	public class DojoSeminarCache : IEnumerable
	{
		...
		public DojoSeminar this[int index]
		{
			get
			{
				lock(this)
				{
					...
					return dojoSeminarArray[i];
				}
			}
		}

		public void CheckedAdd(DojoSeminar dojoSeminar, DateTime expiration)
		{
			lock(this)
			{
		...
		public void Add(DojoSeminar dojoSeminar, DateTime expiration)
		{
			lock(this)
			{
				count++;
		...
		private void quickSort(int left, int right)
		{
			...
			i = left;
			j = right;
			x = dojoSeminarArray[(left + right) / 2].ID;
			while(i <= j)
			{
				while(dojoSeminarArray[i].ID < x & i < right)
					i++;
				while(x < dojoSeminarArray[j].ID & j > left)
					j--;
				if(i <= j)
				{
					ya = dojoSeminarArray[i];
					yb = timeStamps[i];
					yc = absoluteExpirations[i];
					yd = slidingExpirations[i];
					dojoSeminarArray[i] = dojoSeminarArray[j];
					timeStamps[i] = timeStamps[j];
					absoluteExpirations[i] = absoluteExpirations[j];
					slidingExpirations[i] = slidingExpirations[j];
					dojoSeminarArray[j] = ya;
					timeStamps[j] = yb;
		...
		private int binarySearch(int id)
		{
			...
			while (left <= right)
			{
				...
				if(id > dojoSeminarArray[middle].ID)
					left = middle + 1;
				else if(id < dojoSeminarArray[middle].ID)
					right = middle - 1;
				else
					return middle;
			}
