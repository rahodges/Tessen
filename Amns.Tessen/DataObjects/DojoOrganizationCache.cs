...
namespace Amns.GreyFox.Tessen
{
	...
	/// </summary>
	/// </summary>
	public class DojoOrganizationCache : IEnumerable
	{
		...
		public DojoOrganization this[int index]
		{
			get
			{
				lock(this)
				{
					...
					return dojoOrganizationArray[i];
				}
			}
		}

		public void CheckedAdd(DojoOrganization dojoOrganization, DateTime expiration)
		{
			lock(this)
			{
		...
		public void Add(DojoOrganization dojoOrganization, DateTime expiration)
		{
			lock(this)
			{
				count++;
		...
		private void quickSort(int left, int right)
		{
			...
			i = left;
			j = right;
			x = dojoOrganizationArray[(left + right) / 2].ID;
			while(i <= j)
			{
				while(dojoOrganizationArray[i].ID < x & i < right)
					i++;
				while(x < dojoOrganizationArray[j].ID & j > left)
					j--;
				if(i <= j)
				{
					ya = dojoOrganizationArray[i];
					yb = timeStamps[i];
					yc = absoluteExpirations[i];
					yd = slidingExpirations[i];
					dojoOrganizationArray[i] = dojoOrganizationArray[j];
					timeStamps[i] = timeStamps[j];
					absoluteExpirations[i] = absoluteExpirations[j];
					slidingExpirations[i] = slidingExpirations[j];
					dojoOrganizationArray[j] = ya;
					timeStamps[j] = yb;
		...
		private int binarySearch(int id)
		{
			...
			while (left <= right)
			{
				...
				if(id > dojoOrganizationArray[middle].ID)
					left = middle + 1;
				else if(id < dojoOrganizationArray[middle].ID)
					right = middle - 1;
				else
					return middle;
			}
