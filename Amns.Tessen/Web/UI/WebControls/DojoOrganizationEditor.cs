...
namespace Amns.Tessen.Web.UI.WebControls
{
	...
	public class DojoOrganizationEditor : TableWindow, INamingContainer
	{
		...
		protected override void CreateChildControls()
		{
			...
			msClassLocations.Mode = MultiSelectBoxMode.CheckBoxList;
			msClassLocations.ScrollHeight = Unit.Pixel(150);
		...
		private void bindDropDownLists()
		{
			...
			GreyFoxContactCollection administrativeContactCollection = 
				administrativeContactManager.GetCollection(string.Empty, "sysGlobal_Contacts.LastName, sysGlobal_Contacts.FirstName, sysGlobal_Contacts.MiddleName");
			foreach(GreyFoxContact administrativeContact in administrativeContactCollection)
			{
				ListItem i = new ListItem(administrativeContact.ConstructName("L,FM."), administrativeContact.ID.ToString());
			...
			DojoMemberTypeCollection defaultMemberTypeCollection = defaultMemberTypeManager.GetCollection(string.Empty, string.Empty);
		...
		private void renderSystemFolder(HtmlTextWriter output)
		{
			...
			output.WriteAttribute("valign", "top");
			output.Write(HtmlTextWriter.TagRightChar);
			output.Write("Flags the primary organization for Tessen.");
			output.WriteEndTag("td");
			output.WriteBeginTag("td");
			output.Write(HtmlTextWriter.TagRightChar);
